...
import { CheckCircle, Calendar, CakeSlice, ShoppingBag } from 'lucide-react';
...
const Production = () => {
    ...
    const fetchPlan = async () => {
        try {
            const data = await getProductionPlan(date);
            setPlan(data);
        } catch (e) {
            console.error(e);
        } finally {
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="bg-white px-4 py-2 rounded-full border border-border flex items-center gap-2">
                    ...
                    <input 
                        ...
                        className="outline-none bg-transparent font-medium text-primary"
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {plan.length === 0 ? (
                    <div className="col-span-2 text-center py-20 bg-white rounded-xl border border-border">
                        ...
                        <p className="text-muted-foreground">Nessun ordine in attesa di produzione per questa data.</p>
                    </div>
                ) : (
                    plan.map((item, idx) => (
                        <div 
